...
public static class NumberObjectOtigoSessionTracker
{
    private static bool sessionStarted = false;
    private static bool finalSent = false;

    private static int activityId = 7;
    ...
    public static void BeginSession(int newActivityId)
    {
        ...
        Debug.Log("NumberObject OTIGO session başladı. activityId: " + activityId);
    }
    ...
    public static void AddOrUpdateLevelResult(int levelNumber, int durationSeconds, int mistakesMade)
    {
        if (!sessionStarted)
        {
            Debug.LogWarning("NumberObject session başlamadan level result eklenmeye çalışıldı.");
            return;
        }
        ...
        Debug.Log("NumberObject level kaydedildi -> levelNumber: " + levelNumber +
                  ", durationSeconds: " + durationSeconds +
                  ", mistakesMade: " + mistakesMade +
                  ", helpCount: " + helpCount);
    }
    ...
    public static void AddParentHelp()
    {
        if (!sessionStarted)
        {
            Debug.LogWarning("NumberObject session başlamadan parent help eklendi.");
            return;
        }

        totalParentHelpCount++;
        Debug.Log("NumberObject parent help eklendi. Toplam: " + totalParentHelpCount);
    }
    ...
    public static void AddParentHelpForLevel(int levelNumber)
    {
        if (!sessionStarted)
        {
            Debug.LogWarning("NumberObject session başlamadan level bazlı parent help eklendi.");
            return;
        }
        ...
        Debug.Log("NumberObject parent help eklendi -> levelNumber: " + levelNumber +
    ...
    public static void SendFinalResultIfPossible()
    {
        if (!sessionStarted)
        {
            Debug.LogWarning("NumberObject session yok, final sonuç gönderilemez.");
            return;
        }

        if (finalSent)
        {
            Debug.LogWarning("NumberObject final sonuç zaten gönderildi.");
            return;
        }
        ...
        {
            Debug.LogWarning("Hiç NumberObject level sonucu yok, final gönderilmiyor.");
            return;
        }
        ...
        Debug.Log("NumberObject final result hazırlanıyor...");
    ...
    public static void ResetSession()
    {
        ...
        Debug.Log("NumberObject OTIGO session sıfırlandı.");
    }
}