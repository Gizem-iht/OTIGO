...
public static class ObjectSelectOtigoSessionTracker
{
    private static bool sessionStarted = false;
    private static bool finalSent = false;

    private static int activityId = 3;
    ...
    // levelNumber -> target count
    private static Dictionary<int, int> levelTargetCounts
    ...
    public static void BeginSession(int newActivityId)
    {
        ...
        levelResults.Clear();
        levelTargetCounts.Clear();
        levelHelpCounts.Clear();

        Debug.Log("ObjectSelect OTIGO session başladı. activityId: " + activityId);
    }
    ...
    public static void AddParentHelp()
    {
        if (!sessionStarted)
        {
            Debug.LogWarning("ObjectSelect session başlamadan parent help eklendi.");
            return;
        }

        totalParentHelpCount++;
        Debug.Log("ObjectSelect parent help eklendi. Toplam: " + totalParentHelpCount);
    }
    ...
    public static void AddParentHelpForLevel(int levelNumber)
    {
        if (!sessionStarted)
        {
            Debug.LogWarning("ObjectSelect session başlamadan level bazlı parent help eklendi.");
            return;
        }
        ...
        Debug.Log("ObjectSelect parent help eklendi -> levelNumber: " + levelNumber +
                  ", levelHelpCount: " + levelHelpCounts[levelNumber] +
                  ", totalParentHelpCount: " + totalParentHelpCount);
    }

    public static void AddOrUpdateLevelResult(int levelNumber, int durationSeconds, int mistakesMade, int targetCount)
    {
        if (!sessionStarted)
        {
            Debug.LogWarning("ObjectSelect session başlamadan level result eklenmeye çalışıldı.");
            return;
        }
        ...
        levelTargetCounts[levelNumber] = targetCount;

        Debug.Log("ObjectSelect level kaydedildi -> levelNumber: " + levelNumber +
                  ", durationSeconds: " + durationSeconds +
                  ", mistakesMade: " + mistakesMade +
                  ", helpCount: " + helpCount +
    ...
    public static void SendFinalResultIfPossible()
    {
        ...
        int totalTargetCount = levelTargetCounts.Values.Sum();
        int maxLevelPlayed = orderedResults.Max(x => x.levelNumber);

        Debug.Log("ObjectSelect final result hazırlanıyor...");
    ...
    public static void ResetSession()
    {
        ...
        Debug.Log("ObjectSelect OTIGO session sıfırlandı.");
    }
}