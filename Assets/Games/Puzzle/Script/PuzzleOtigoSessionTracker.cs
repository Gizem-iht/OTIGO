...
public static class PuzzleOtigoSessionTracker
{
    private static bool sessionStarted = false;
    private static bool finalSent = false;

    private static int activityId = 8;
    ...
    public static void BeginSession(int newActivityId)
    {
        ...
        Debug.Log("Puzzle OTIGO session başladı. activityId: " + activityId);
    }
    ...
    public static void AddParentHelp()
    {
        if (!sessionStarted)
        {
            Debug.LogWarning("Puzzle session başlamadan parent help eklendi.");
            return;
        }

        totalParentHelpCount++;
        Debug.Log("Puzzle parent help eklendi. Toplam: " + totalParentHelpCount);
    }
    ...
    public static void AddParentHelpForLevel(int levelNumber)
    {
        if (!sessionStarted)
        {
            Debug.LogWarning("Puzzle session başlamadan level bazlı parent help eklendi.");
            return;
        }
        ...
        Debug.Log("Puzzle parent help eklendi -> levelNumber: " + levelNumber +
    ...
    public static void AddOrUpdateLevelResult(int levelNumber, int durationSeconds, int mistakesMade)
    {
        if (!sessionStarted)
        {
            Debug.LogWarning("Puzzle session başlamadan level result eklenmeye çalışıldı.");
            return;
        }
        ...
        Debug.Log("Puzzle level kaydedildi -> levelNumber: " + levelNumber +
                  ", durationSeconds: " + durationSeconds +
                  ", mistakesMade: " + mistakesMade +
                  ", helpCount: " + helpCount);
    }
    ...
    public static void SendFinalResultIfPossible()
    {
        ...
        Debug.Log("Puzzle final result hazırlanıyor...");
    ...
    public static void ResetSession()
    {
        ...
        Debug.Log("Puzzle OTIGO session sıfırlandı.");
    }
}