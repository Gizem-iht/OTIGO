...
public class MazeLevel7Controller : MonoBehaviour
{
    [Header("UI")]
    ...
    public string nextLevelSceneName = "Maze_TebrikScene";

    [Header("Toothbrush")]
    public ToothbrushDrag toothbrush;
    public Transform brushStartPoint;
    ...
    public AudioClip eyeWrongClip;
    public AudioClip earWrongClip;
    ...
    private void Start()
    {
        ...
        if (toothbrush != null)
        {
            toothbrush.controller = this;
            toothbrush.wallLayers = wallLayers;
            toothbrush.Unlock();
        }
    ...
    public void OnHitWall()
    {
        ...
        StartCoroutine(ResetBrushAfterDelay());
    }

    public void OnEyeWrong()
    {
        if (finished || inPenalty) return;
        ...
        if (voiceSource != null && eyeWrongClip != null)
            voiceSource.PlayOneShot(eyeWrongClip);

        StartCoroutine(ResetBrushAfterDelay());
    }

    public void OnEarWrong()
    {
        if (finished || inPenalty) return;
        ...
        if (voiceSource != null && earWrongClip != null)
            voiceSource.PlayOneShot(earWrongClip);

        StartCoroutine(ResetBrushAfterDelay());
    }

    private IEnumerator ResetBrushAfterDelay()
    {
        if (toothbrush == null || brushStartPoint == null)
        {
            inPenalty = false;
            yield break;
        }
        ...
        Rigidbody2D rb = toothbrush.GetComponent<Rigidbody2D>();
        ...
        if (rb != null)
        {
            ...
            rb.position = brushStartPoint.position;
        }
        else
        {
            toothbrush.transform.position = brushStartPoint.position;
        }
    ...
    private void SaveLevelResultAndSendFinal()
    {
        ...
        MazeOtigoSessionTracker.SendFinalResultIfPossible();
    }
    ...
    public void AddParentHelp()
    {
        ...
        Debug.Log("Maze Level 7 parentHelpCount arttı: " + parentHelpCount);
    }
    ...
    private void SaveState()
    {
        ...
        if (toothbrush != null)
        {
            Vector3 pos = toothbrush.transform.position;

            PlayerPrefs.SetFloat(StatePrefix + "BrushX", pos.x);
            PlayerPrefs.SetFloat(StatePrefix + "BrushY", pos.y);
            PlayerPrefs.SetFloat(StatePrefix + "BrushZ", pos.z);
        }
    ...
    private void LoadState()
    {
        ...
        if (toothbrush != null)
        {
            float x = PlayerPrefs.GetFloat(StatePrefix + "BrushX", toothbrush.transform.position.x);
            float y = PlayerPrefs.GetFloat(StatePrefix + "BrushY", toothbrush.transform.position.y);
            float z = PlayerPrefs.GetFloat(StatePrefix + "BrushZ", toothbrush.transform.position.z);

            toothbrush.transform.position = new Vector3(x, y, z);
    ...
    private void ClearState()
    {
        ...
        PlayerPrefs.DeleteKey(StatePrefix + "BrushX");
        PlayerPrefs.DeleteKey(StatePrefix + "BrushY");
        PlayerPrefs.DeleteKey(StatePrefix + "BrushZ");
