...
/// <summary>
/// Shadow Match Level 4 (<c>ShadowMatch_Level4</c>): <c>game</c> altındaki oyun kökleri (ör. Arac + Shadow)
/// Variant_L4_A / Variant_L4_B olarak sarılır ve randomizer bağlanır.
/// </summary>
public static class ShadowMatchLevel4VariantSetup
{
    private const string Level4ScenePath = "Assets/Games/Shadow/level3/ShadowMatch_Level4.unity";

    private const string GameplayRootName = "game";

    [MenuItem("OTIGO/Shadow/Level 4 — Kur: Varyant A+B + Randomizer")]
    public static void Setup()
    {
        if (!EditorUtility.DisplayDialog(
                "Shadow Level 4",
                "ShadowMatch_Level4 sahnesinde \"" + GameplayRootName +
                "\" objesinin doğrudan çocukları Variant_L4_A altına alınır, kopya Variant_L4_B " +
                "oluşturulur ve Randomizer eklenir. Devam?",
                "Evet",
                "İptal"))
            return;

        var scene = EditorSceneManager.OpenScene(Level4ScenePath, OpenSceneMode.Single);
        ...
        if (root.Find("Variant_L4_A") != null)
        {
            EditorUtility.DisplayDialog(
                "Shadow Level 4",
                "Variant_L4_A zaten kurulu. Tekrar kurmak için Variant_L4_A/B ve Level4_VariantRandom " +
                "objelerini sil veya Undo ile geri al.",
                "Tamam");
            return;
        }

        DestroyIfExists(GameObject.Find("Level4_VariantRandom"));
        ...
        var variantA = new GameObject("Variant_L4_A");
        ...
        variantB.name = "Variant_L4_B";

        TintColoredPiecesSlightly(variantB);

        variantB.SetActive(false);

        GameObject rnd = new GameObject("Level4_VariantRandom");
        ...
        EditorUtility.DisplayDialog(
            "Bitti",
            "Level 4 varyant yapısı kaydedildi.\n\n" +
            "• Variant_L4_B şimdilik kopya + hafif renk tonudur.\n" +
            "• Farklı nesneler için Variant_L4_B içinde sprite’ları değiştir.\n\n" +
