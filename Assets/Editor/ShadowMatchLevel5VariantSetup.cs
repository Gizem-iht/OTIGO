...
/// <summary>
/// Shadow Match Level 5 (<c>ShadowMatch_Level5</c>): <c>game</c> altındaki oyun kökleri
/// Variant_L5_A / Variant_L5_B olarak sarılır ve randomizer bağlanır.
/// </summary>
public static class ShadowMatchLevel5VariantSetup
{
    private const string Level5ScenePath = "Assets/Games/Shadow/level5/ShadowMatch_Level5.unity";

    private const string GameplayRootName = "game";

    [MenuItem("OTIGO/Shadow/Level 5 — Kur: Varyant A+B + Randomizer")]
    public static void Setup()
    {
        if (!EditorUtility.DisplayDialog(
                "Shadow Level 5",
                "ShadowMatch_Level5 sahnesinde \"" + GameplayRootName +
                "\" objesinin doğrudan çocukları Variant_L5_A altına alınır, kopya Variant_L5_B " +
                "oluşturulur ve Randomizer eklenir. Devam?",
                "Evet",
                "İptal"))
            return;

        var scene = EditorSceneManager.OpenScene(Level5ScenePath, OpenSceneMode.Single);
        ...
        if (root.Find("Variant_L5_A") != null)
        {
            EditorUtility.DisplayDialog(
                "Shadow Level 5",
                "Variant_L5_A zaten kurulu. Tekrar kurmak için Variant_L5_A/B ve Level5_VariantRandom " +
                "objelerini sil veya Undo ile geri al.",
                "Tamam");
            return;
        }

        DestroyIfExists(GameObject.Find("Level5_VariantRandom"));
        ...
        var variantA = new GameObject("Variant_L5_A");
        ...
        variantB.name = "Variant_L5_B";

        TintColoredPiecesSlightly(variantB);
        variantB.SetActive(false);

        GameObject rnd = new GameObject("Level5_VariantRandom");
        ...
        EditorUtility.DisplayDialog(
            "Bitti",
            "Level 5 varyant yapısı kaydedildi.\n\n" +
            "• Variant_L5_B şimdilik kopya + hafif renk tonudur.\n" +
            "• Farklı nesneler için Variant_L5_B içinde sprite’ları değiştir.\n\n" +
