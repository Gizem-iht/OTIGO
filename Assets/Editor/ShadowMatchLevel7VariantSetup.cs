...
/// <summary>
/// Shadow Match Level 7 (<c>ShadowMatch_Level7</c>): <c>game</c> altındaki oyun kökleri
/// Variant_L7_A / Variant_L7_B olarak sarılır ve randomizer bağlanır.
/// </summary>
public static class ShadowMatchLevel7VariantSetup
{
    private const string Level7ScenePath = "Assets/Games/Shadow/level7/ShadowMatch_Level7.unity";

    private const string GameplayRootName = "game";

    [MenuItem("OTIGO/Shadow/Level 7 — Kur: Varyant A+B + Randomizer")]
    public static void Setup()
    {
        if (!EditorUtility.DisplayDialog(
                "Shadow Level 7",
                "ShadowMatch_Level7 sahnesinde \"" + GameplayRootName +
                "\" objesinin doğrudan çocukları Variant_L7_A altına alınır, kopya Variant_L7_B " +
                "oluşturulur ve Randomizer eklenir. Devam?",
                "Evet",
                "İptal"))
            return;

        var scene = EditorSceneManager.OpenScene(Level7ScenePath, OpenSceneMode.Single);
        ...
        if (root.Find("Variant_L7_A") != null)
        {
            EditorUtility.DisplayDialog(
                "Shadow Level 7",
                "Variant_L7_A zaten kurulu. Tekrar kurmak için Variant_L7_A/B ve Level7_VariantRandom " +
                "objelerini sil veya Undo ile geri al.",
                "Tamam");
            return;
        }

        DestroyIfExists(GameObject.Find("Level7_VariantRandom"));
        ...
        var variantA = new GameObject("Variant_L7_A");
        ...
        variantB.name = "Variant_L7_B";

        TintColoredPiecesSlightly(variantB);
        variantB.SetActive(false);

        GameObject rnd = new GameObject("Level7_VariantRandom");
        ...
        EditorUtility.DisplayDialog(
            "Bitti",
            "Level 7 varyant yapısı kaydedildi.\n\n" +
            "• Variant_L7_B şimdilik kopya + hafif renk tonudur.\n" +
            "• Farklı nesneler için Variant_L7_B içinde sprite’ları değiştir.\n\n" +
